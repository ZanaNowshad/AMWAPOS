    ...
    <div className="login">
      <section className="login-brand">
        ...
        <div className="meta">
          ...
          <div>{t("Version {0}", status.app_version)}</div>
        ...
        <div className="login-panel">
          {!selected ? (
            <div className="stack-24">
              <div>
                <h1>{t("Who is signing in?")}</h1>
                <div className="muted">{t("Select your name, then enter your PIN.")}</div>
              </div>
            ...
            <div className="pin-panel">
              ...
              <div className="col" style={{ alignItems: "center", margin: "12px 0 16px" }}>
                ...
                <span className="tiny">{tb(selected.role_name)}</span>
              </div>
              {selected.locked ? (
                <Banner tone="danger">
                  {t("This account is locked after too many incorrect PINs. Ask a manager to unlock it, or wait.")}
                </Banner>
              ) : null}
              <PinEntry title={t("Enter your PIN")} onSubmit={submit} busy={busy} error={error} />
            </div>
          )}
        </div>
      </section>
