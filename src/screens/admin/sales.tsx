...
export function SaleDrawer({ saleId, onClose }: { saleId: string; onClose: () => void }) {
  ...
  return (
    <Drawer
      title={s ? `Receipt ${s.receipt_number}` : "Sale"}
      onClose={onClose}
      actions={
        has("pos.reprint") ? (
          <Button
          ...
          {tab === "receipt" ? (
            ...
            <>
              ...
              <table className="table">
                ...
                <tfoot>
                  <tr>
                    <td colSpan={4}>Total (VAT {formatMoney(s.tax_minor)})</td>
                    <td />
              ...
              <div>
                ...
                {s.payments.map((p, i) => (
                  <div key={i} className="row small">
                    <span className="grow">
                      {methodLabel(p.method)} {p.reference ? `· ref ${p.reference}` : ""}
                    </span>
                    <span className="num">
              ...
              {s.refunds.length ? (
                <div>
                  <h3 style={{ marginBottom: 6 }}>Refunds</h3>
                  {s.refunds.map((r) => (
...
export function RefundsPage() {
  ...
  return (
    <div>
      <PageHeader title="Refunds" subtitle="Every refund with its reason, operator and approver." />
      <div className="filters">
...
export function ShiftsPage() {
  ...
  return (
    <div>
      <PageHeader title="Shifts" subtitle="Cash reconciliation per cashier shift." />
      <div className="filters">
        <DateRange from={from} to={to} onChange={(a, b) => (setFrom(a), setTo(b))} />
      </div>
      {error ? <Banner tone="danger">{error}</Banner> : null}
      <DataTable<ShiftSummary>
        ...
        rowKey={(r) => r.shift_id}
        onRowClick={setOpen}
        empty={<div className="empty">No shifts in this period.</div>}
        columns={[
          { key: "n", label: "Shift", render: (r) => <span className="mono">{r.shift_number}</span> },
          { key: "c", label: "Cashier", render: (r) => r.cashier_name, sort: (r) => r.cashier_name },
          { key: "t", label: "Terminal", render: (r) => r.device_name ?? "—" },
          { key: "o", label: "Opened", render: (r) => formatShort(r.opened_at), sort: (r) => r.opened_at },
      ...
      />
      {open ? (
        <Drawer title={`Shift ${open.shift_number}`} onClose={() => setOpen(null)}>
          <ShiftDetail s={open} />
        </Drawer>
      ) : null}
    </div>
  ...
  return (
    <div className="stack-16">
      <dl className="kv">
        ...
        <dt>Variance approved by</dt>
        <dd>{s.variance_approved_by_name ?? "—"}</dd>
        <dt>Note</dt>
        <dd>{s.close_note ?? "—"}</dd>
      </dl>
      <div>
        <h3 style={{ marginBottom: 6 }}>Cash events</h3>
        {(events ?? []).length === 0 ? <div className="small muted">None.</div> : null}
        {(events ?? []).map((e) => (
    ...
    <div>
      ...
      <DataTable<R>
        ...
        columns={[
          ...
          { key: "u", label: "User", render: (r) => r.user_name ?? "—" },
          { key: "ap", label: "Approver", render: (r) => r.approver_name ?? "—" },
          { key: "s", label: "Shift", render: (r) => <span className="mono">{r.shift_number}</span> },
        ]}
      />
    </div>
  );
