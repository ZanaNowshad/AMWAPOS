...
function CustomerForm({ initial, onSaved, onCancel }: { initial: CustomerRow | null; onSaved: (c: CustomerRow) => void; onCancel: () => void }) {
  const [f, setF] = useState<CustomerInput>(initial ?? { name: "", phone: "", whatsapp: "", email: "", area: "", address: "", active: true });
  const act = useAction();
  const set = (k: keyof CustomerInput, v: string | boolean) => setF({ ...f, [k]: v });
  return (
    <div className="col gap-16">
      <div className="form-grid">
        <TextInput label="Name" required value={f.name} onChange={(e) => set("name", e.target.value)} fieldClass="span-2" autoFocus />
        <TextInput label="Phone" value={f.phone ?? ""} onChange={(e) => set("phone", e.target.value)} hint="8-digit Bahrain numbers get +973." />
      ...
      <div className="row">
        ...
        <Button
          ...
          loading={act.busy}
          onClick={async () => {
            const r = await act.run(() => api.customers.save(initial?.customer_id ?? null, { ...f, phone: f.phone || null, whatsapp: f.whatsapp || null, email: f.email || null, area: f.area || null, address: f.address || null }));
            if (r) onSaved(r);
          }}
        >
          Save
  ...
  return (
    <div>
      ...
      <div className="filters">
        <input className="input" style={{ width: 300 }} placeholder="Search phone or name…" value={q} onChange={(e) => setQ(e.target.value)} />
      </div>
      ...
      <DataTable<CustomerRow>
        ...
        columns={[
          ...
          { key: "l", label: "Last Purchase", render: (r) => (r.last_purchase_at ? relative(r.last_purchase_at) : "—"), sort: (r) => r.last_purchase_at ?? "" },
          { key: "c", label: "Purchases", num: true, render: (r) => r.purchase_count, sort: (r) => r.purchase_count },
          { key: "t", label: "Total Purchases", num: true, render: (r) => <Money minor={r.total_spent_minor} />, sort: (r) => r.total_spent_minor },
      ...
      {creating ? (
        <Drawer title="New customer" onClose={() => setCreating(false)}>
          <CustomerForm initial={null} onCancel={() => setCreating(false)} onSaved={(c) => (setCreating(false), void reload(), nav(`/admin/customers/${c.customer_id}`))} />
        </Drawer>
      ) : null}
    </div>
  );
  ...
  return (
    <div>
      <div className="page-header">
        <Button variant="ghost" icon={<ArrowLeft size={18} />} aria-label="Back" onClick={() => nav("/admin/customers")} />
        <div className="grow">
      ...
      {tab === "purchases" ? (
        <DataTable<Record<string, unknown>>
          ...
          empty={<div className="empty">{has("sales.view") ? "No purchases yet." : "Your role cannot view purchase history."}</div>}
          columns={[
      ...
      {tab === "notes" ? (
        <div className="col gap-16">
          {has("customers.manage") ? (
            <div className="row">
              <input className="input grow" placeholder="Add a note…" value={note} onChange={(e) => setNote(e.target.value)} />
              <Button
                variant="primary"
                disabled={!note.trim()}
                onClick={async () => {
      ...
      {editing ? (
        <Drawer title={`Edit ${c.name}`} onClose={() => setEditing(false)}>
          <CustomerForm initial={c} onCancel={() => setEditing(false)} onSaved={() => (setEditing(false), void reload())} />
        </Drawer>
      ) : null}
...
const DSTATUS: Record<string, "default" | "info" | "warning" | "success"> = { pending: "warning", preparing: "info", dispatched: "info", delivered: "success", cancelled: "default" };
  ...
  return (
    <DataTable<DeliveryRow>
      ...
      columns={[
        ...
        { key: "p", label: "Payment", render: (r) => <Chip tone={r.payment_status === "paid" ? "success" : "warning"}>{payLabel(r.payment_status)}</Chip> },
...
export function DeliveriesPage() {
  ...
  const detail = useLoad(() => (open ? api.deliveries.get(open.delivery_id) : Promise.resolve(null)), [open?.delivery_id]);
  const act = useAction();
          ...
          {(["pending", "preparing", "dispatched"] as const).map((st) => (
            <div key={st} className="kanban-col">
              <h3 style={{ margin: "4px 4px 10px" }}>
                {st[0].toUpperCase() + st.slice(1)} <span className="tiny">({data.filter((d) => d.status === st).length})</span>
              </h3>
              {data
                .filter((d) => d.status === st)
                .map((d) => (
                  <div key={d.delivery_id} className="kanban-card" onClick={() => setOpen(d)} tabIndex={0} role="button">
                    <div className="row">
                    ...
                    <div className="row" style={{ marginTop: 6 }}>
                      ...
                      <Chip tone={d.payment_status === "paid" ? "success" : "warning"}>{payLabel(d.payment_status)}</Chip>
                    </div>
                  </div>
                ))}
            </div>
        ...
        <Drawer title={`Delivery ${open.delivery_number}`} onClose={() => setOpen(null)}>
          <div className="stack-16">
            ...
            {has("deliveries.manage") ? (
              <div className="col">
                <Field label="Assign rider">
                  <select className="select" value={open.assigned_user_id ?? ""} onChange={(e) => update({ delivery_id: open.delivery_id, assigned_user_id: e.target.value })}>
                ...
                <Field label="Payment">
                  <select className="select" value={open.payment_status} onChange={(e) => update({ delivery_id: open.delivery_id, payment_status: e.target.value })}>
            ...
            <div className="row wrap">
              {open.status === "pending" ? <Button onClick={() => update({ delivery_id: open.delivery_id, status: "preparing" })}>Preparing</Button> : null}
              {open.status === "pending" || open.status === "preparing" ? (
                <Button variant="primary" onClick={() => update({ delivery_id: open.delivery_id, status: "dispatched" })}>
                  Dispatch
                </Button>
              ) : null}
              {open.status === "dispatched" ? (
                <Button variant="primary" onClick={() => update({ delivery_id: open.delivery_id, status: "delivered" })}>
                  Delivered
                </Button>
              ) : null}
              {["pending", "preparing", "dispatched"].includes(open.status) && has("deliveries.manage") ? (
                <Button variant="danger-outline" onClick={() => update({ delivery_id: open.delivery_id, status: "cancelled" })}>
                  Cancel
                </Button>
              ) : null}
            </div>
